...
function isEndOfImageMarker(dataSet, position) {
  return (
    dataSet.byteArray[position] === 0xff &&
...
function isFragmentEndOfImage(dataSet, pixelDataElement, fragmentIndex) {
  ...
  if (
    isEndOfImageMarker(dataSet, fragment.position + fragment.length - 2) ||
...
function findLastImageFrameFragmentIndex(
  dataSet,
  pixelDataElement,
  startFragment,
) {
  for (
    var fragmentIndex = startFragment;
    fragmentIndex < pixelDataElement.fragments.length;
    fragmentIndex++
  ) {
 ...
 */
export default function createJPEGBasicOffsetTable(
  dataSet,
  pixelDataElement,
  fragments,
) {
  ...
  while (true) {
    ...
    basicOffsetTable.push(
      pixelDataElement.fragments[startFragmentIndex].offset,
    );
    var endFragmentIndex = findLastImageFrameFragmentIndex(
      dataSet,
      pixelDataElement,
      startFragmentIndex,
    );
    ...
    if (
      endFragmentIndex === undefined ||
      endFragmentIndex === pixelDataElement.fragments.length - 1
    ) {
