 ...
 */
export function parseDicomDataSetExplicit(
  dataSet,
  byteStream,
  maxPosition,
  options = {},
  ...
  maxPosition =
    maxPosition === undefined ? byteStream.byteArray.length : maxPosition;
  ...
  if (
    maxPosition < byteStream.position ||
 ...
 * @param maxPosition the maximum position to read up to (optional - only needed when reading sequence items)
 */
export function parseDicomDataSetImplicit(
  dataSet,
  byteStream,
  maxPosition,
  options = {},
  ...
  maxPosition =
    maxPosition === undefined ? dataSet.byteArray.length : maxPosition;
  ...
  while (byteStream.position < maxPosition) {
    const element = readDicomElementImplicit(
      byteStream,
      options.untilTag,
      options.vrCallback,
    );
