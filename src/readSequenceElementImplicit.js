...
function readSequenceItemImplicit(byteStream, vrCallback) {
  ...
  if (item.length === 4294967295) {
    ...
    item.dataSet = readDicomDataSetImplicitUndefinedLength(
      byteStream,
      vrCallback,
    );
    ...
    parseDicomDataSet.parseDicomDataSetImplicit(
      item.dataSet,
      byteStream,
      byteStream.position + item.length,
      { vrCallback },
    );
  }
 ...
 */
export default function readSequenceItemsImplicit(
  byteStream,
  element,
  vrCallback,
) {
