 ...
 */
export default function readEncapsulatedPixelDataFromFragments(
  dataSet,
  pixelDataElement,
  startFragmentIndex,
  numFragments,
  ...
  if (numFragments === 1) {
    return sharedCopy(
      ...
      fragmentZeroPosition +
        fragments[startFragmentIndex].offset +
        fragmentHeaderSize,
  ...
  for (let i = startFragmentIndex; i < startFragmentIndex + numFragments; i++) {
    let fragmentOffset =
      fragmentZeroPosition + fragments[i].offset + fragmentHeaderSize;
